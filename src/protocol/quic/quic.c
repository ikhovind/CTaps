...
ct_quic_send_state_t* ct_quic_send_state_new(uv_buf_t* buffer, picoquic_cnx_t* cnx,
                                             struct sockaddr_storage local_address,
                                             struct sockaddr_storage remote_address,
                                             ct_connection_group_t* connection_group) {
...
picoquic_quic_t* ct_connection_get_picoquic_instance(ct_connection_t* connection) {
    if (!connection || !connection->socket_manager ||
        !connection->socket_manager->internal_socket_manager_state) {
        log_error("ct_quic_connection_get_picoquic_instance called with NULL connection or missing "
...
void socket_timer_close_cb(uv_handle_t* handle) {
    ...
    if (quic_ctx->ticket_store_path) {
        ...
        if (rc != 0) {
            log_error("Failed to save QUIC session tickets to store %s: %d",
                      quic_ctx->ticket_store_path, rc);
        } else {
            log_trace("Successfully saved QUIC session tickets to store %s",
                      quic_ctx->ticket_store_path);
        }
    }
...
ct_quic_socket_state_t* ct_quic_socket_state_new(
    ...
    if (!cert_file || !key_file || !security_parameters) {
        log_error("Certificate, key files and security parameters are required for QUIC context "
                  "creation");
        return NULL;
    }

    const char* ticket_store_path =
        ct_security_parameters_get_ticket_store_path(security_parameters);
    ...
    size_t stek_len = 0;
    const uint8_t* stek =
        ct_security_parameters_get_session_ticket_encryption_key(security_parameters, &stek_len);
    log_info("Stek address: %p", (void*)stek);
    if (stek) {
        log_trace("Using session ticket encryption key of length %zu from security parameters",
                  ticket_key_length);
        ticket_key = stek;
    ...
    socket_state->picoquic_ctx = picoquic_create(
        MAX_CONCURRENT_QUIC_CONNECTIONS, socket_state->cert_file_name, socket_state->key_file_name,
        NULL, alpn_strings[0], picoquic_callback, socket_state, NULL, NULL, NULL,
        picoquic_current_time(), NULL, ticket_store_path, ticket_key, ticket_key_length);
...
void ct_quic_set_connection_stream(ct_connection_t* connection, uint64_t stream_id) {
    ct_quic_stream_state_t* stream_state = ct_connection_get_stream_state(connection);
    if (!stream_state) {
        log_error("Cannot set conncetion stream for connection %s, stream state is NULL",
                  connection->uuid);
        return;
    }

    log_debug("Setting QUIC stream ID %llu for connection %s", (unsigned long long)stream_id,
              connection->uuid);
...
void ct_connection_assign_next_free_stream(ct_connection_t* connection, bool is_unidirectional) {
    ct_quic_connection_group_state_t* group_state =
        connection->connection_group->connection_group_state;
    ...
    log_debug("Assigned QUIC stream ID: %llu (unidirectional: %d)",
              (unsigned long long)next_stream_id, is_unidirectional);
...
ct_connection_get_quic_group_state(const ct_connection_t* connection) {
    if (!connection || !connection->connection_group ||
        !connection->connection_group->connection_group_state) {
        log_error("Cannot get QUIC group state, connection or group state is NULL");
        log_debug("conn=%p, group=%p, group_state=%p", (void*)connection,
...
ct_quic_stream_state_t* ct_connection_get_stream_state(const ct_connection_t* connection) {
    if (!connection || !connection->internal_connection_state) {
        ...
        log_debug("conn=%p, internal_state=%p", (void*)connection,
...
picoquic_cnx_t* ct_connection_get_picoquic_connection(const ct_connection_t* connection) {
    ...
    assert(group_state);
    log_debug("Retrieved picoquic connection %p for connection",
              (void*)group_state->picoquic_connection);
...
ct_quic_socket_state_t* ct_connection_get_quic_socket_state(const ct_connection_t* connection) {
    if (!connection || !connection->socket_manager) {
        ...
        log_debug("Connection: %p, connection->socket_manager: %p", connection,
                  connection ? connection->socket_manager : NULL);
        return NULL;
    }
...
void reset_quic_timer(ct_quic_socket_state_t* quic_context) {
    if (!quic_context || !quic_context->picoquic_ctx || !quic_context->timer_handle) {
        ...
        log_debug("ctx=%p, ctx->quic_ctx=%p, ctx->timer_handle=%p", (void*)quic_context,
                  (void*)(quic_context ? quic_context->picoquic_ctx : NULL),
                  (void*)(quic_context ? quic_context->timer_handle : NULL));
        return;
    }
    uint64_t next_wake_delay = picoquic_get_next_wake_delay(
        quic_context->picoquic_ctx, picoquic_get_quic_time(quic_context->picoquic_ctx),
        INT64_MAX - 1);
    log_trace("Resetting QUIC timer to fire in %llu ms",
              (unsigned long long)MICRO_TO_MILLI(next_wake_delay));
...
int handle_closed_picoquic_connection(ct_connection_group_t* connection_group) {
    log_debug("Handling closed picoquic connection for connection group %s",
...
static int handle_stream_data(ct_connection_t* connection, const uint8_t* bytes, size_t length) {
    ...
    if (!ct_connection_can_receive(connection)) {
        log_error("Received data on stream after FIN was already received for connection %s",
                  connection->uuid);
        return -EPIPE;
    }
...
static void handle_stream_fin(ct_connection_t* connection) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (ct_connection_can_send(conn) || ct_connection_can_receive(conn)) {
            log_debug("Connection %s is still active (can_send=%d, can_receive=%d)", conn->uuid,
                      ct_connection_can_send(conn), ct_connection_can_receive(conn));
            num_active++;
        }
    }

    if (num_active == 0) {
        log_debug("No more active connections in group after receiving FIN, closing entire QUIC "
                  "connection");
        ct_quic_connection_group_state_t* group_state =
            ct_connection_get_quic_group_state(connection);
        ...
        log_info("Both send and receive sides closed for connection %s, closing connection",
                 connection->uuid);
...
static int resolve_or_create_stream_connection(picoquic_cnx_t* cnx,
                                               ct_connection_group_t* connection_group,
                                               void* v_stream_ctx, uint64_t stream_id,
    ...
    if (!connection) {
        ...
        if (ct_connection_stream_is_initialized(connection)) {
            ...
            *out_is_new_connection = true;
            connection =
                ct_connection_create_clone(connection, connection->socket_manager,
                                           connection->framer_impl, ct_quic_stream_state_new());
            if (!connection) {
        ...
        if (picoquic_is_client(cnx)) {
            log_debug("New stream %llu is for client connection, determining send/receive "
                      "capabilities based on stream ID",
                      (unsigned long long)stream_id);
            can_receive = server_initiated || !unidirectional;
            can_send = !server_initiated || !unidirectional;
        } else {
            log_debug("New stream %llu is for server connection, determining send/receive "
                      "capabilities based on stream ID",
                      (unsigned long long)stream_id);
            can_receive = !server_initiated || !unidirectional;
            can_send = server_initiated || !unidirectional;
        }
        log_debug("Stream %llu capabilities - can_send: %d, can_receive: %d",
                  (unsigned long long)stream_id, can_send, can_receive);
...
int picoquic_callback(picoquic_cnx_t* cnx, uint64_t stream_id, uint8_t* bytes, size_t length,
    ...
    case picoquic_callback_ready:
        ...
        if (ct_connection_is_server(connection)) {
            ...
            log_debug("Number of connections in socket manager: %zu",
                      g_slist_length(socket_manager->all_connections));
    ...
    case picoquic_callback_stream_data: {
        ...
        int rc = resolve_or_create_stream_connection(cnx, connection_group, v_stream_ctx, stream_id,
    ...
    case picoquic_callback_stream_reset:
        ...
        if (v_stream_ctx) {
            ...
            log_warn("Received RESET on stream %llu but no stream context available",
                     (unsigned long long)stream_id);
        }
        break;
    case picoquic_callback_stop_sending:
        ...
        if (v_stream_ctx) {
            ...
            log_warn("Received STOP_SENDING on stream %llu but no stream context available",
                     (unsigned long long)stream_id);
        }
        break;
    ...
    case picoquic_callback_close:
        ...
        if (error != 0) {
            ...
            log_debug("Closing connection group %s due to peer close with error",
                      connection_group->connection_group_id);
            rc = handle_aborted_picoquic_connection_group(connection_group);
        } else {
            log_debug("QUIC connection %s closed by peer without error",
                      connection_group->connection_group_id);
...
static int ct_quic_send_packet_with_pktinfo(ct_udp_poll_handle_t* poll_handle,
    ...
    log_debug("Sending QUIC packet from %s:%d to %s:%d with pktinfo on interface index %d", from_ip,
              from_port, to_ip, to_port, if_index);
...
void on_quic_poll_read(ct_socket_manager_t* socket_manager, const uint8_t* buf, ssize_t nread,
    ...
    int rc = picoquic_incoming_packet_ex(picoquic_ctx, (uint8_t*)buf, nread,
                                         (struct sockaddr*)addr_from, (struct sockaddr*)addr_to, 0,
                                         0, &cnx, picoquic_get_quic_time(picoquic_ctx));
    if (rc != 0) {
        log_error("Error processing incoming QUIC packet: %d", rc);
    }

    // Same new-connection handling as when first reading
    if (cnx && picoquic_get_callback_context(cnx) ==
        ...
        if (!socket_manager->listener || ct_listener_is_closed(socket_manager->listener)) {
            log_debug(
                "Received packet for QUIC cnx but no listener available to accept it, ignoring");
            return;
        }
        ...
        ct_quic_socket_state_t* socket_state =
            (ct_quic_socket_state_t*)listener->socket_manager->internal_socket_manager_state;
...
int quic_init_with_send(ct_connection_t* connection, ct_message_t* initial_message,
    ...
    size_t bundle_count =
        ct_security_parameters_get_client_certificate_count(connection->security_parameters);
    ...
    const char* cert_file =
        ct_security_parameters_get_client_certificate_file(connection->security_parameters, 0);
    const char* key_file =
        ct_security_parameters_get_client_certificate_key_file(connection->security_parameters, 0);
    ...
    ct_udp_poll_handle_t* poll_handle =
        create_udp_poll_on_local(ct_connection_get_active_local_endpoint(connection));
    if (!poll_handle) {
    ...
    ct_quic_connection_group_state_t* group_state =
        (ct_quic_connection_group_state_t*)connection->connection_group->connection_group_state;
    ...
    log_trace("Setting callback context to connection group: %p",
              (void*)connection->connection_group);

    // Set picoquic callback to connection group
    picoquic_set_callback(group_state->picoquic_connection, picoquic_callback,
                          connection->connection_group);

    bool is_final = initial_message_context &&
                    ct_message_properties_get_final(
                        ct_message_context_get_message_properties(initial_message_context));

    ct_connection_assign_next_free_stream(connection, false);
    rc = picoquic_add_to_stream_with_ctx(
        group_state->picoquic_connection, ct_connection_get_stream_id(connection),
        (const uint8_t*)initial_message->content, initial_message->length, is_final, connection);
    ...
    rc = picoquic_set_app_stream_ctx(group_state->picoquic_connection,
                                     ct_connection_get_stream_id(connection), connection);
...
int quic_close_connection(ct_connection_t* connection) {
    ...
    if (num_active > 0) {
        ...
        if (ct_connection_stream_is_initialized(connection)) {
            ...
            rc = picoquic_add_to_stream_with_ctx(group_state->picoquic_connection, stream_id, NULL,
                                                 0, 1, connection);
...
void quic_abort(ct_connection_t* connection) {
    ...
    uint64_t stream_id = ct_connection_get_stream_id(connection);
    ct_connection_group_t* connection_group = connection->connection_group;
    uint64_t num_active_connections =
        ct_connection_group_get_num_active_connections(connection_group);

    log_info("Aborting connection using QUIC, active connections in group: %u",
    ...
    if (num_active_connections > 1) {
        ...
        if (ct_connection_stream_is_initialized(connection)) {
            log_debug("Sending RST on stream %llu for connection: %s",
                      (unsigned long long)stream_id, connection->uuid);
            int rc = picoquic_reset_stream(group_state->picoquic_connection, stream_id, 0);
            if (rc != 0) {
                log_error("Error sending RST on stream %llu: %d", (unsigned long long)stream_id,
                          rc);
                return;
            }
    ...
    reset_quic_timer(
        (ct_quic_socket_state_t*)connection->socket_manager->internal_socket_manager_state);
}

int quic_clone_connection(const struct ct_connection_s* source_connection,
...
int quic_send(ct_connection_t* connection, ct_message_t* message,
    ...
    if (!ct_connection_stream_is_initialized(connection)) {
        log_debug("First message sent on QUIC stream for connection %s, initializing stream",
                  connection->uuid);
    ...
    uint64_t stream_id = ct_connection_get_stream_id(connection);
    log_debug("Queuing %zu bytes for QUIC, sending on stream %llu, connection: %s", message->length,
              (unsigned long long)stream_id, connection->uuid);

    int set_fin = 0;
    if (message_context && ct_message_properties_get_final(&message_context->message_properties)) {
        log_debug("Setting FIN on QUIC stream %llu for connection: %s",
                  (unsigned long long)stream_id, connection->uuid);
        set_fin = 1;
    }

    int rc = picoquic_add_to_stream_with_ctx(cnx, stream_id, (const uint8_t*)message->content,
                                             message->length, set_fin, connection);
...
int quic_listen(ct_socket_manager_t* socket_manager) {
    ...
    size_t bundle_count =
        ct_security_parameters_get_server_certificate_count(listener->security_parameters);
    ...
    const char* cert_file =
        ct_security_parameters_get_server_certificate_file(listener->security_parameters, 0);
    const char* key_file =
        ct_security_parameters_get_server_certificate_key_file(listener->security_parameters, 0);
...
void quic_free_state(ct_connection_t* connection) {
    ...
    ct_quic_stream_state_t* stream_state =
        (ct_quic_stream_state_t*)connection->internal_connection_state;
    free(stream_state);
}
...
int quic_set_connection_priority(ct_connection_t* connection, uint8_t priority) {
    ...
    if (!stream_state->stream_initialized) {
        log_warn(
            "Attempting to set QUIC stream priority before stream is initialized for connection %s",
            connection->uuid);
        return -EINVAL;
    }
...
void quic_free_socket_state(struct ct_socket_manager_s* socket_manager) {
    ct_quic_socket_state_t* socket_state =
        (ct_quic_socket_state_t*)socket_manager->internal_socket_manager_state;
