...
ct_tcp_send_data_t* ct_tcp_send_data_new(ct_connection_t* connection, ct_message_t* message,
...
void on_clone_connect(struct uv_connect_s* req, int status) {
    ...
    int rc = uv_read_start((uv_stream_t*)socket_state->tcp_handle, alloc_cb, tcp_on_read);
    if (rc < 0) {
...
void on_write(uv_write_t* req, int status) {
    ...
    if (status < 0) {
        ...
        socket_manager->callbacks.message_send_error(connection, send_data->message_context,
                                                     status);
    } else {
...
int tcp_init_with_send(ct_connection_t* connection, ct_message_t* initial_message,
    ...
    connection->socket_manager->internal_socket_manager_state = ct_tcp_socket_state_new(
        connection, NULL, initial_message, initial_message_context, connect_req, new_tcp_handle);
    ...
    rc = uv_tcp_connect(connect_req, new_tcp_handle,
                        (const struct sockaddr*)remote_endpoint_get_resolved_address(
...
int tcp_init(ct_connection_t* connection) {
    ...
    connection->socket_manager->internal_socket_manager_state =
        ct_tcp_socket_state_new(connection, NULL, NULL, NULL, connect_req, new_tcp_handle);
    if (!connection->socket_manager->internal_socket_manager_state) {
        log_error("Failed to allocate memory for TCP connection state");
        free(connect_req);
        uv_close((uv_handle_t*)new_tcp_handle, on_libuv_close);
    ...
    if (tmp.ss_family == AF_INET) {
        ...
        log_debug("Connecting to remote endpoint with unsupported address family %d via TCP",
                  tmp.ss_family);
    }
...
int tcp_listen(ct_socket_manager_t* socket_manager) {
    ...
    socket_manager->internal_socket_manager_state =
        ct_tcp_socket_state_new(NULL, listener, NULL, NULL, NULL, new_tcp_handle);
...
void new_stream_connection_cb(uv_stream_t* server, int status) {
    ...
    ct_socket_manager_t* server_conn_socket_manager =
        ct_socket_manager_new(&tcp_protocol_interface, NULL);
    ...
    server_conn_socket_manager->internal_socket_manager_state =
        ct_tcp_socket_state_new(server_conn, listener, NULL, NULL, NULL, server_conn_tcp_handle);
...
int tcp_clone_connection(const struct ct_connection_s* source_connection,
    ...
    if (target_connection->socket_manager) {
        target_connection->socket_manager->all_connections =
            g_slist_remove(target_connection->socket_manager->all_connections, target_connection);
    ...
    socket_manager->internal_socket_manager_state =
        ct_tcp_socket_state_new(target_connection, NULL, NULL, NULL, connect_req, new_tcp_handle);
    new_tcp_handle->data = socket_manager;
    ...
    rc = uv_tcp_connect(connect_req, new_tcp_handle,
                        (const struct sockaddr*)remote_endpoint_get_resolved_address(
                            ct_connection_get_active_remote_endpoint(target_connection)),
                        on_clone_connect);
