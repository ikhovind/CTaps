...
static int ct_string_array_value_deep_copy(const ct_string_array_t source,
                                           ct_string_array_t* dest) {
    ...
    for (size_t i = 0; i < SEC_PROPERTY_END; i++) {
        ...
        if (src_param->set_by_user) {
            ...
            case TYPE_STRING_ARRAY: {
                ...
                int rc = ct_string_array_value_deep_copy(src_param->value.array_of_strings,
                                                         &dst_param->value.array_of_strings);
                if (rc < 0) {
            ...
            case TYPE_CERTIFICATE_BUNDLES: {
                int rc = ct_certificate_bundles_deep_copy(src_param->value.certificate_bundles,
                                                          &dst_param->value.certificate_bundles);
                if (rc < 0) {
            ...
            case TYPE_BYTE_ARRAY:
                ...
                memcpy(dst_param->value.byte_array.bytes, src_param->value.byte_array.bytes,
                       src_param->value.byte_array.length);
...
int ct_security_parameters_set_ticket_store_path(ct_security_parameters_t* sec,
...
int ct_security_parameters_add_to_array_of_strings(ct_security_parameters_t* sec,
                                                   ct_security_property_enum_t type,
                                                   const char* value) {
    ...
    sec->list[type].value.array_of_strings.strings = new_strings;
    new_strings[prev_arr.num_strings] = strdup(value);

    if (!new_strings[prev_arr.num_strings]) {
        log_error("Could not allocate memory for server certificate file");
        free(new_strings[prev_arr.num_strings]);
        return -ENOMEM;
    }
    sec->list[type].value.array_of_strings.num_strings++;
...
int ct_security_parameters_set_session_ticket_encryption_key(ct_security_parameters_t* sec,
...
int ct_security_parameters_add_certificate(ct_security_parameters_t* sec,
                                           ct_security_property_enum_t type, const char* cert_file,
                                           const char* key_file) {
    if (!sec || !cert_file) {
    ...
    ct_certificate_bundle_t* bundle_array =
        realloc(prev_bundles.certificate_bundles,
                (prev_bundles.num_bundles + 1) * sizeof(ct_certificate_bundle_t));
    if (!bundle_array) {
        log_error("Could not allocate memory for server certificate array");
        return -ENOMEM;
    }
...
int ct_security_parameters_add_server_certificate(ct_security_parameters_t* sec,
                                                  const char* cert_file, const char* key_file) {
    return ct_security_parameters_add_certificate(sec, SERVER_CERTIFICATE, cert_file, key_file);
}

int ct_security_parameters_add_client_certificate(ct_security_parameters_t* sec,
                                                  const char* cert_file, const char* key_file) {
    return ct_security_parameters_add_certificate(sec, CLIENT_CERTIFICATE, cert_file, key_file);
}

const char* ct_security_parameters_get_certificate_file(const ct_security_parameters_t* sec,
                                                        ct_security_property_enum_t type,
                                                        size_t index) {
    if (!sec) {
    ...
    return sec->list[type]
        .value.certificate_bundles.certificate_bundles[index]
        .certificate_file_name;
}

const char* ct_security_parameters_get_key_file(const ct_security_parameters_t* sec,
                                                ct_security_property_enum_t type, size_t index) {
    if (!sec) {
        log_warn("Attempting to get key file from NULL security parameters");
        return NULL;
    }
    if (sec->list[type].value.certificate_bundles.num_bundles <= index) {
        log_warn("Key file with index %zu does not exist", index);
        return NULL;
    }
    return sec->list[type]
        .value.certificate_bundles.certificate_bundles[index]
...
const char* ct_security_parameters_get_server_certificate_file(const ct_security_parameters_t* sec,
                                                               size_t index) {
...
size_t ct_security_parameters_get_client_certificate_count(const ct_security_parameters_t* sec) {
    if (!sec) {
        log_warn("Attempting to get client certificate count from NULL security parameters");
        return 0;
    }
    return sec->list[CLIENT_CERTIFICATE].value.certificate_bundles.num_bundles;
}

const char* ct_security_parameters_get_client_certificate_file(const ct_security_parameters_t* sec,
                                                               size_t index) {
...
int ct_security_parameters_set_server_name_identification(
    ct_security_parameters_t* security_parameters, const char* sni) {
    if (!security_parameters) {
...
int ct_security_parameters_add_supported_group(ct_security_parameters_t* sec, const char* group) {
    return ct_security_parameters_add_to_array_of_strings(sec, SUPPORTED_GROUP, group);
}
