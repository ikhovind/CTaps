 ...
 */
typedef enum {
    get_security_parameter_array_list(output_enum) get_security_parameter_scalar_list(output_enum)
        SEC_PROPERTY_END
...
#define create_sec_property_initializer(enum_name, string_name, property_type, token_name,         \
                                        default_value, type_tag)                                   \
    [enum_name] = {.name = (string_name), .type = (type_tag), .set_by_user = false, .value = {{0}}},

static const ct_security_parameters_t DEFAULT_SECURITY_PARAMETERS = {
    .list = {get_security_parameter_array_list(create_sec_property_initializer)
 ...
 */
typedef enum {
    get_selection_property_list(output_enum) get_preference_set_selection_property_list(output_enum)
 ...
 */
typedef enum {
    get_message_property_list(output_enum) MESSAGE_PROPERTY_END
} ct_message_properties_enum_t;
...
typedef enum {
    get_writable_connection_property_list(output_enum)
        get_read_only_connection_properties(output_enum) get_tcp_connection_properties(output_enum)
...
typedef struct ct_listener_s {
    ...
    ct_connection_callbacks_t
        connection_callbacks; ///< User-provided callbacks for connection events on accepted connections
    ct_listener_state_enum_t state; ///< Current state of the listener
    ct_security_parameters_t*
        security_parameters; ///< Security configuration for accepted connections (owned copy)
...
typedef struct ct_socket_manager_s {
    ...
    GHashTable*
        demux_table; // remote_endpoint → ct_connection_t* (Only used for UDP where demultiplexing is needed)
...
typedef struct ct_per_connection_properties_s {
    ...
    uint8_t
        priority; ///< Relative priority of this compared to others in the same connection group, 0 is highest.
    bool can_receive;
    bool can_send;
