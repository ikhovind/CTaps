...
ct_local_endpoint_t* ct_local_endpoint_resolve(const ct_local_endpoint_t* local_endpoint,
    ...
    *out_count = 0;
    ...
    log_debug("Found %d addresses for interface %s", num_found_addresses,
              local_endpoint->interface_name ? local_endpoint->interface_name : "any");
    ...
    for (size_t i = 0; i < *out_count; i++) {
        ...
        if (ss->ss_family == AF_INET) {
            ...
            inet_ntop(AF_INET6, &((struct sockaddr_in6*)ss)->sin6_addr, addr_str, sizeof(addr_str));
        } else {
            snprintf(addr_str, sizeof(addr_str), "<unknown family %d>", ss->ss_family);
        }
        log_debug("Resolved local endpoint [%zu]: %s:%u (iface: %s)", i, addr_str, out_list[i].port,
                  out_list[i].interface_name ? out_list[i].interface_name : "any");
    }
...
ct_local_endpoint_t* ct_local_endpoints_deep_copy(const ct_local_endpoint_t* local_endpoints,
    ...
    for (size_t i = 0; i < num_local_endpoints; i++) {
        ...
        if (rc != 0) {
            log_error("Failed to copy content for local endpoint at index %zu: %s", i,
                      strerror(-rc));
...
int ct_local_endpoint_from_sockaddr(ct_local_endpoint_t* local_endpoint,
    ...
    log_trace("Building local endpoint from sockaddr");
    if (!addr) {
        log_error("Cannot build local endpoint from NULL sockaddr");
        return -EINVAL;
    }
    if (local_endpoint->service != NULL) {
        log_error("Cannot specify both service and IP address on single local endpoint");
        return -EINVAL;
    }
...
bool ct_local_endpoint_resolved_equals(const ct_local_endpoint_t* endpoint1,
                                       const ct_local_endpoint_t* endpoint2) {
