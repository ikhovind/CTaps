...
int ct_remote_endpoint_resolve(const ct_remote_endpoint_t* remote_endpoint,
    ...
    if (assigned_port == -1) {
        log_error("Could not determine port for remote endpoint with service %s",
...
ct_remote_endpoint_t* ct_remote_endpoints_deep_copy(const ct_remote_endpoint_t* remote_endpoints,
    ...
    ct_remote_endpoint_t* res_endpoints =
        calloc(num_remote_endpoints, sizeof(ct_remote_endpoint_t));
    if (!res_endpoints) {
    ...
    for (size_t i = 0; i < num_remote_endpoints; i++) {
        ...
        if (rc != 0) {
            log_error("Failed to copy content for remote endpoint at index %zu: %s", i,
                      strerror(-rc));
