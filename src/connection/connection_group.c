...
int ct_connection_group_add_connection(ct_connection_group_t* group, ct_connection_t* connection) {
    ...
    if (connection->connection_group) {
        log_error("Connection with UUID %s already belonged to a connection group",
                  connection->uuid);
        return -EEXIST;
    }
...
int ct_connection_group_remove_connection(ct_connection_group_t* group,
    ...
    log_debug("Connection removed, remaining connections in group: %u",
              g_hash_table_size(group->connections));
    return 0;
}
...
ct_connection_t** ct_connection_get_grouped_connections(const ct_connection_t* connection,
                                                        size_t* out_count) {
    ...
    log_debug("Returning %zu active connections from group %s", num_active,
...
void ct_connection_group_unref(const ct_connection_t* connection) {
    ...
    log_trace("Unrefing connection group %s with ref count: %u", group->connection_group_id,
              group->ref_count);
    group->ref_count--;
    if (group->ref_count == 0) {
        log_debug("Connection group %s ref count is zero, freeing connection group",
                  group->connection_group_id);
