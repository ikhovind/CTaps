...
ct_transport_properties_t* ct_transport_properties_new(void) {
    ...
    memcpy(&props->selection_properties, &DEFAULT_SELECTION_PROPERTIES,
           sizeof(ct_selection_properties_t));
    memcpy(&props->connection_properties, &DEFAULT_CONNECTION_PROPERTIES,
...
int ct_transport_properties_add_preference_map(ct_transport_properties_t* transport_props,
    ...
    ct_preference_combination_t* tmp = realloc(
        set->combinations, (set->num_combinations + 1) * sizeof(ct_preference_combination_t));
    if (!tmp) {
...
ct_transport_properties_get_interface_preference(const ct_transport_properties_t* transport_props,
    ...
    for (size_t i = 0; i < transport_props->selection_properties.list[INTERFACE]
                               .value.preference_set_val.num_combinations;
         i++) {
        if (strcmp(value, transport_props->selection_properties.list[INTERFACE]
                              .value.preference_set_val.combinations[i]
                              .value) == 0) {
            return transport_props->selection_properties.list[INTERFACE]
                .value.preference_set_val.combinations[i]
                .preference;
        }
    }
...
ct_transport_properties_get_pvd_preference(const ct_transport_properties_t* transport_props,
    ...
    for (size_t i = 0;
         i <
         transport_props->selection_properties.list[PVD].value.preference_set_val.num_combinations;
         i++) {
        if (strcmp(value, transport_props->selection_properties.list[PVD]
                              .value.preference_set_val.combinations[i]
                              .value) == 0) {
            return transport_props->selection_properties.list[PVD]
                .value.preference_set_val.combinations[i]
                .preference;
        }
    }
