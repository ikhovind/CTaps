...
TEST_F(UdpListenTests, canReceiveAfterListenerCLose) {
    ct_local_endpoint_t* listener_endpoint = ct_local_endpoint_new();
    ct_local_endpoint_with_interface(listener_endpoint, "lo");
    ct_local_endpoint_with_port(listener_endpoint, 1239);
    ...
    EXPECT_EQ(per_connection_messages[server_connection][0]->length, 5);
    EXPECT_STREQ(per_connection_messages[server_connection][0]->content, "ping");
