...
TEST_F(QuicMigrationTest, migrationFailsIfMulipathIsNotActive) {
    struct IptablesGuard guard = IptablesGuard();
    struct IpAddressGuard ipAddr = IpAddressGuard();
    ct_remote_endpoint_t* remote_endpoint = ct_remote_endpoint_new();
    ct_remote_endpoint_with_port(remote_endpoint, QUIC_PING_PORT);
    ...
    EXPECT_EQ(connection->num_remote_endpoints, 1);
    EXPECT_EQ(connection->num_local_endpoints, 2);

    EXPECT_TRUE(ct_connection_is_closed(connection));
    EXPECT_EQ(per_connection_messages[connection].size(), 1);
    EXPECT_STREQ(per_connection_messages[connection][0]->content, "Pong: ping");

    EXPECT_EQ(test_context.connection_path_change_count[connection], 0) << "Expected no path change event when not allowed";

    EXPECT_EQ(test_context.local_sockaddr.size(), 1);
    EXPECT_FALSE(test_context.connection_succeeded);
