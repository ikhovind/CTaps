...
TEST_F(RemoteEndpointDnsTests, canDnsLookupHostName) {
  ...
  ASSERT_NE(transport_properties, nullptr);
    ...
    // check address port
    if (ct_connection_get_active_remote_endpoint(saved_connection)->resolved_address.ss_family == AF_INET) {
        ...
        struct sockaddr_in6* addr = (struct sockaddr_in6*)&ct_connection_get_active_remote_endpoint(saved_connection)->resolved_address;
